function Set-ExcelDefaultOption {
  ...
  .DESCRIPTION
    ...
    ftypeを実行するためには管理者権限でコマンドを実行する必要がある
  ...
  .PARAMETER WithoutStartUpScreen
    起動画面を表示しないようにする
  ...
  .EXAMPLE
    PS> Set-ExcelDefaultOption -IsReadOnly -WithoutStartUpScreen
      Excelを開いたときの挙動を読み取り専用かつ起動画面を表示しないようにする
  ...
  Param(
    ...
    [switch] $WithoutStartUpScreen = $false
  )
  ...
  # optionの設定
  $options = ''
  if($WithoutStartUpScreen) { $options += ' /e' }
  if($IsReadOnly) { $options += ' /r' }

  # ファイル関連付けの全取得
  cmd /c assoc | Where-Object   {
    # 関連付けセットにExcelが入ってるものを抽出（大文字小文字を区別）
    $_ -clike '*Excel*'
  } | ForEach-Object {
    # ファイルタイプ部分を抽出
    $_.Split('=')[1]
